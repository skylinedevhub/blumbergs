# Snapshot Generator Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create `scripts/reports/generate_snapshot.py` that produces Excel workbooks with T3010 line totals for 13 scopes (Canada, 9 provinces, 3 designations).

**Architecture:** Single parameterized Python script. A `get_scope_bn_clause()` function returns a SQL subquery of BNs in scope. Each sheet-builder function queries one or more tables, JOINing against the scope. Uses openpyxl for Excel output. CLI via argparse.

**Tech Stack:** Python 3, DuckDB, openpyxl (all already installed)

---

## Task 1: Scaffold script with CLI and scope filter

**Files:**
- Create: `scripts/reports/generate_snapshot.py`

**Step 1: Create the script with imports, constants, CLI, and scope filter**
...
**Step 2: Test scaffold runs and produces placeholder workbooks**

Run: `python3 scripts/reports/generate_snapshot.py`
Expected: Creates `data/exports/snapshots_2024/snapshot_2024_canada.xlsx` with placeholder sheets.

Run: `python3 scripts/reports/generate_snapshot.py --all`
Expected: Creates all 13 workbooks with placeholder sheets.

**Step 3: Commit scaffold**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: scaffold snapshot generator with CLI and scope filtering"
```

---

## Task 2: Implement build_section_a (Section A - Identification)

**Files:**
- Modify: `scripts/reports/generate_snapshot.py` — replace `build_section_a` placeholder

**Step 1: Replace the build_section_a function**

This queries `ident`, `charity_base`, and `financial_abc` for identification fields.
...
Run: `python3 scripts/reports/generate_snapshot.py`
Expected: Section A sheet populated with charity counts, contact info, A1-A3 yes/no counts.

Verify against known 2024 values: total = 83,275 (ident) or 83,093 (financial_d).

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Section A identification sheet"
```

---

## Task 3: Implement build_section_c (Section C - Programs & General Info)

**Files:**
- Modify: `scripts/reports/generate_snapshot.py` — replace `build_section_c` placeholder

**Step 1: Replace build_section_c**
...
def build_section_c(wb, con, where):
    ...
    ongoing = con.execute(f"""
        ...
        INNER JOIN charity_base cb ON t."BN/Registration number" = cb.bn
        WHERE {where} AND t."Program Type" = 'O'
    """).fetchone()[0]
    new_prog = con.execute(f"""
        SELECT COUNT(*) FROM programs t
        INNER JOIN charity_base cb ON t."BN/Registration number" = cb.bn
        WHERE {where} AND t."Program Type" = 'N'
    """).fetchone()[0]
...
Run: `python3 scripts/reports/generate_snapshot.py`
Verify Section C sheet has all C1-C18 fields populated, fundraising method counts match known values.

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Section C programs and general info sheet"
```

---

## Task 4: Implement build_section_d (Section D - Financial Summary)

**Files:**
- Modify: `scripts/reports/generate_snapshot.py` — replace `build_section_d` placeholder

**Step 1: Replace build_section_d**
...
def build_section_d(wb, con, where):
    ...
    def yn_line(col, table="financial_d"):
        """Count accrual/cash or yes/no responses."""
        bn = BN_COL[table]
        y = con.execute(f"""
    ...
    accrual = con.execute(f"""
        SELECT COUNT(*) FROM {st('financial_d', where)} AND t."4020" = 'Accrual'
    """).fetchone()[0]
    cash = con.execute(f"""
        SELECT COUNT(*) FROM {st('financial_d', where)} AND t."4020" = 'Cash'
    """).fetchone()[0]
...
Run: `python3 scripts/reports/generate_snapshot.py`
Verify Canada total revenue (~$439B) matches existing workbook.

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Section D financial summary sheet"
```

---

## Task 5: Implement build_schedule_1 and build_schedule_2

**Files:**
- Modify: `scripts/reports/generate_snapshot.py`

**Step 1: Replace both schedule builders**
...
Run: `python3 scripts/reports/generate_snapshot.py`
Verify Schedule 1 and Schedule 2 sheets populated. Foreign spending ~$5.1B for Canada.

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Schedule 1 (foundations) and Schedule 2 (foreign activities)"
```

---

## Task 6: Implement build_schedule_3 (Compensation)

**Files:**
- Modify: `scripts/reports/generate_snapshot.py`

**Step 1: Replace build_schedule_3**
...
def build_schedule_3(wb, con, where):
    ...
    val_4880 = con.execute(
        f"SELECT SUM({money('t.\"4880\"')}) FROM {st('financial_d', where)}"
    ).fetchone()[0]
...
Run: `python3 scripts/reports/generate_snapshot.py`
Verify total compensation ~$231B for Canada, FT positions, salary bands.

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Schedule 3 compensation sheet"
```

---

## Task 7: Implement build_schedule_5 (Non-cash gifts)

**Files:**
- Modify: `scripts/reports/generate_snapshot.py`

**Step 1: Replace build_schedule_5**
...
Run: `python3 scripts/reports/generate_snapshot.py`
Verify non-cash gift counts and total ~$6B.

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Schedule 5 non-cash gifts sheet"
```

---

## Task 8: Implement build_schedule_6 (Detailed Financials — the big one)

**Files:**
- Modify: `scripts/reports/generate_snapshot.py`

**Step 1: Replace build_schedule_6**

This is the largest sheet — all ~70 line items from Schedule 6.
...
def build_schedule_6(wb, con, where):
    ...
    ws.append([])
    ws.append(["4250", "Property not used in charitable activities", s("4250") if "4250" in [c[0] for c in con.execute("DESCRIBE financial_d").fetchall()] else None])
...
Run: `python3 scripts/reports/generate_snapshot.py`
Verify Schedule 6 total assets ~$798.8B, total revenue ~$439.1B, total expenditures ~$405.5B.

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Schedule 6 detailed financials sheet"
```

---

## Task 9: Implement build_schedule_8 (Disbursement Quota)

**Files:**
- Modify: `scripts/reports/generate_snapshot.py`

**Step 1: Replace build_schedule_8**
...
Run: `python3 scripts/reports/generate_snapshot.py`
Verify Schedule 8 populated with DQ calculations.

**Step 3: Commit**

```bash
git add scripts/reports/generate_snapshot.py
git commit -m "feat: implement Schedule 8 disbursement quota sheet"
```

---

## Task 10: Generate all 13 workbooks and validate
...
Expected: 13 workbooks in `data/exports/snapshots_2024/`.

**Step 2: Spot-check Canada workbook against existing**

Compare `snapshot_2024_canada.xlsx` Schedule 6 values against `data/exports/blumbergs_snapshot_2024.xlsx` "Schedule 6 - Financials" sheet. Key values to verify:
- Line 4200 total assets: should be ~$798.8B
- Line 4700 total revenue: should be ~$439.1B
- Line 5100 total expenditures: should be ~$405.5B
- Line 390 total compensation: should be ~$231B

**Step 3: Spot-check one provincial workbook**

Open `snapshot_2024_ON.xlsx`, verify Section A shows ~30,421 charities for Ontario.

**Step 4: Spot-check one designation workbook**

Open `snapshot_2024_designation_A.xlsx`, verify Section A shows ~4,619 public foundations.

**Step 5: Commit final state**

```bash
git add scripts/reports/generate_snapshot.py docs/plans/2026-02-23-snapshot-generator-design.md docs/plans/2026-02-23-snapshot-generator.md
git commit -m "feat: complete snapshot generator — 13 workbooks (Canada, 9 provincial, 3 designation)"
```

---

## Reference: Key Expected Values (2024 Canada)

From existing `blumbergs_snapshot_2024.xlsx`:

| Metric | Expected |
|--------|----------|
| Total charities (ident) | 83,275 |
| Charities in financial_d | 83,093 |
| Total revenue (4700) | $439,086,935,797 |
| Total expenditures (5100) | $405,474,714,707 |
| Total assets (4200) | $798,757,232,979 |
| Total liabilities (4350) | $423,124,496,735 |
| Total compensation (390) | $231,019,010,839 |
| Foreign activities (Sch2 line 200) | $5,086,546,381 |
| FT positions (300) | 2,925,494 |
| PT positions (370) | 3,111,411 |
| DAF accounts (5861) | 186,405 |
